  ...
  useEffect(() => {
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      recognitionRef.current.onerror = (event) => {
        ...
        // НЕ переключаємось автоматично на бекенд при "network" error!
        // Це може бути тимчасова проблема, а не відсутність мікрофона
        ...
        if (event.error === 'no-speech') {
          ...
          // Мережева помилка Web Speech API - НЕ переходимо на бекенд!
          console.log('[WebSpeech] Мережева помилка Web Speech API');
          setError('⚠️ Помилка голосового сервісу.\n\nСпробуйте:\n1. Перевірте інтернет-з\'єднання\n2. Натисніть "Спробувати ще" нижче\n\nАбо використовуйте режим "Демо"');
        } else {
          console.log(`[WebSpeech] Інша помилка: ${event.error}`);
          setError(`❌ Помилка розпізнавання: ${event.error}\n\nСпробуйте ще раз або використайте режим "Демо"`);
        }
      };
